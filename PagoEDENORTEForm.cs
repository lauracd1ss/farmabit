...
namespace farmabit
{
    public partial class PagoEDENORTEForm : Form
    {
        public PagoEDENORTEForm()
        {
            InitializeComponent();
        }
        SqlConnection conexion = new SqlConnection("server=localhost\\MSSQLSERVER2022;database=bose;integrated security=true");

        private void PagoEDENORTEForm_Load(object sender, EventArgs e)
        {
        ...
        {
            if (string.IsNullOrWhiteSpace(txtcon.Text) || string.IsNullOrWhiteSpace(textBox3.Text))
            {
            ...
            VALUES (@Agencia, @NumeroContrato, @MontoTotal, @FechaEmision, @Titular)";
                ...
                ejecutar.Parameters.AddWithValue("@NumeroContrato", txtcon.Text ?? string.Empty);
